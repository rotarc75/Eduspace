...
const COLORS_MAP = { blue: '#7B9CF4', green: '#5BC8A0', purple: '#B08CF4', amber: '#F4B860', red: '#F47B7B' }

function AddSubjectModal({ open, onClose }) {
  const { addSubject } = useApp()
  const [name, setName] = useState(''); const [color, setColor] = useState('blue'); const [saving, setSaving] = useState(false)
  async function handleAdd() {
    if (!name.trim() || saving) return
    setSaving(true); await addSubject({ name: name.trim(), color }); setSaving(false)
    setName(''); setColor('blue'); onClose()
  }
  return (
    <Modal open={open} onClose={onClose} title="Nouvelle matière" size="sm">
      <div className="form-stack">
        <div className="form-group">
          ...
          <input placeholder="Ex : Physique, Anglais…" value={name} autoFocus
            onChange={e => setName(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleAdd()} />
        </div>
        <div className="form-group">
          <label className="form-label">Couleur</label>
          <div style={{ display: 'flex', gap: 8 }}>
            {Object.entries(COLORS_MAP).map(([c, hex]) => (
              <button key={c} type="button" onClick={() => setColor(c)}
                style={{ width: 32, height: 32, borderRadius: '50%', border: `3px solid ${color === c ? hex : 'transparent'}`, background: hex, padding: 0, opacity: color === c ? 1 : 0.4, transition: 'opacity 0.15s, border-color 0.15s' }} />
            ))}
          </div>
        </div>
...
function Workspace({ isProf }) {
  const { tickets, devoirs, subjects, nextSession, loading, dbError } = useApp()
  const { studentName, profName, backToDashboard, logout } = useAuth()
  ...
  const pendingDevoirs = (devoirs ?? []).filter(d => !d.done && d.deadline && new Date(d.deadline) >= new Date(new Date().toDateString())).length
  ...
  return (
    <div className="app-layout">
      <Sidebar
        ...
        onBackToDashboard={isProf ? backToDashboard : null}
        onLogout={logout}
      />
